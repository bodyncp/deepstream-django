...
from machine_vision import settings
...
def get_phone_secure(title, field):
    """
    对手机号进行脱敏操作。
    ...
    def inner(self, obj=None, is_header=None, *args, **kwargs):
        if is_header:
            return title
        contact_info_queryset = str(getattr(obj, field))
        contact_info_head = contact_info_queryset[0:3]
        contact_info_end = contact_info_queryset[7::]
        out_contact_info = contact_info_head + "*" * 4 + contact_info_end

        return out_contact_info
...
class StarkModelForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for name, field in self.fields.items():
            # if TypedChoiceField != type(field):
            field.widget.attrs['class'] = 'form-control'
            # else:
            #     # field.widget.attrs['class'] = 'radio radio-inline'
            #     field.widget.attrs['class'] = 'form-control'
...
class StarkHandler(object):
    ...
    def get_download_btn(self, request, *args, **kwargs):
        permission_dict = request.session.get(settings.PERMISSION_SESSION_KEY)
        if "download_file" not in permission_dict:
            return None

        if request.path_info == "/stark/vision/personpostinfo/list/":
            return "<a class='btn btn-primary' href='/download/'>导出数据</a>"
        else:
            return None
    ...
    def changelist_view(self, request, *args, **kwargs):
        ...
        # 时间模糊搜索搜索
        fuzzy_time = request.GET.get('time')
        fuzzy_time_list = str(fuzzy_time).split("-")
        fuzzy_search_list = self.get_fuzzy_search()
        if fuzzy_search_list and fuzzy_time:
            queryset = queryset.filter(registration_time__year=fuzzy_time_list[0],
                                       registration_time__month=fuzzy_time_list[1],
                                       registration_time__day=fuzzy_time_list[2]).order_by(*order_list)

        ########### 下载 ###########
        queryset_dict["download_data"] = queryset
        ...
        download_btn = self.get_download_btn(request, *args, **kwargs)
        ...
        # for i in search_group_row_list:
        #     for j in i:
        #         print(j)
        return render(
            request,
    ...
    def change_view(self, request, pk, *args, **kwargs):
        ...
        if request.method == 'GET':
            ...
            return render(
                request, self.change_template or 'stark/change.html',
                {
                    'form': form,
                    "users": user_name,
                    "rest_password": request.session.get("rest_password"),
    ...
    def get_region_select_id(self, path_info):
        path_info_list = path_info.split("/")
        show_region = path_info_list[-3]
        region_id = models.ShowRegion.objects.filter(nick_name=show_region).first()
        return region_id
